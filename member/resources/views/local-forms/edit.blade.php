...
@section('content')
    <h2>✏️ Fill Out Local Travel Form</h2>
    ...
    {{-- ✅ Attachment uploader - only if submitted --}}
                ...
                @foreach($form->attachments as $file)
                    <li>
                        ...
                        <form action="{{ route('attachments.delete', $file->id) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('DELETE')
                            <button type="submit" onclick="return confirm('Remove this file?')" style="color: red;">🗑️</button>
                        </form>
                    </li>
                @endforeach
            </ul>
    ...
    {{-- ✅ Cancel the form --}}
    <form method="POST" action="{{ route('member.local-forms.cancel', $form->id) }}" 
          onsubmit="return confirm('Are you sure you want to cancel this travel form?');" 
