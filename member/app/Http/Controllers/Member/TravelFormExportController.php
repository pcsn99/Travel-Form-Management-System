...
use App\Mail\OverseasFormExportMail;
...
class TravelFormExportController extends Controller
{
    ...
    public function emailExport(Request $request, $id)
    {
        ...
        $filename = 'overseas_travel_form_' . $form->id . '_emailed.xlsx';
        ...
        $directorEmail = $form->request->user->director_email ?? 'director@example.com';
        $userEmail = $form->request->user->email;
        ...
        return back()->with('success', 'Export emailed to Director of Works.');
    }
    ...
    public function exportOverseas(Request $request, $id)
    {
        ...
        if (is_null($signature)) {
            if (!$request->has('force')) {
                return redirect()->route('Overseas-forms.show', $id)->with('warning', 'The user has not submitted a signature. Do you still want to proceed with the export? 
                    <a href="' . route('admin.overseas-forms.export', ['id' => $id, 'force' => 1]) . '" class="btn btn-sm btn-warning ms-2">Yes, proceed</a>');
            }
        }
        ...
        if ($form->status === 'approved') {
            ...
            if ($approver && $approver->signature) {
                $adminSignaturePath = public_path('shared/' . $approver->signature);

                if (file_exists($adminSignaturePath)) {
                    $adminDrawing = new Drawing();
                    $adminDrawing->setName('Approver Signature');
                    $adminDrawing->setDescription('Admin Signature');
                    $adminDrawing->setPath($adminSignaturePath);
                    $adminDrawing->setHeight(50);
                    $adminDrawing->setCoordinates('A30');
        ...
        $filename = 'overseas_travel_form_' . $form->id . '.xlsx';
        $path = storage_path('app/public/' . $filename);
    ...
    private function generateSpreadsheet($form)
    {
        $spreadsheet = new Spreadsheet();
        $sheet = $spreadsheet->getActiveSheet();
        ...
        // Signature
        $signature = $form->request->user->signature ?? null;
        if ($signature && file_exists(public_path('shared/' . $signature))) {
            $drawing = new Drawing();
            $drawing->setName('Signature');
            $drawing->setDescription('User Signature');
            $drawing->setPath(public_path('shared/' . $signature));
